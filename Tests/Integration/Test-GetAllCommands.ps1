...
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseBOMForUnicodeEncodedFile', '', Justification='Cross-platform compatibility')]
param(
...
function Invoke-TestCommand {
    ...
    try {
        ...
        $TestResult = Build-TestResult -Command $CommandName -ParameterSet $ParameterSetName -Status "Success" -Message "Retrieved $ResultCount items" -ResultCount $ResultCount -Duration $Duration -SampleData $SampleData
        ...
        $TestResult = Build-TestResult -Command $CommandName -ParameterSet $ParameterSetName -Status "Failed" -Message $ErrorMessage -Duration $Duration -ErrorDetails $_
...
if ($TestStoreId) {
    ...
    $TestResults += Build-TestResult -Command "Get-Filter" -ParameterSet "ByStoreId" -Status "Skipped" -Message "No store ID available for filter test"
}
    ...
    if ($Secrets -and $Secrets.Count -gt 0) {
        ...
        if ($TestSecretId) {
            ...
            if ($Detailed) {
                ...
                $TestResults += Build-TestResult -Command "Get-SecretValue" -ParameterSet "ById" -Status "Skipped" -Message "Skipped for security (use -Detailed to enable)"
            }
        } else {
            $TestResults += Build-TestResult -Command "Get-SecretValue" -ParameterSet "ById" -Status "Skipped" -Message "No valid secret ID found in secret objects"
        }
    } else {
        $TestResults += Build-TestResult -Command "Get-SecretValue" -ParameterSet "ById" -Status "Skipped" -Message "No secrets available for value test"
    }
} catch {
    $TestResults += Build-TestResult -Command "Get-SecretValue" -ParameterSet "ById" -Status "Skipped" -Message "Error retrieving secrets for ID test: $($_.Exception.Message)"
}
